...
static const char websocket[] = "ws://";
...
xmrig::Url::Url(const char *url) { parse(url); }

xmrig::Url::Url(const char *host, uint16_t port, bool tls, Scheme scheme)
    : m_tls(tls), m_scheme(scheme), m_host(host), m_port(port) {
...
bool xmrig::Url::isEqual(const Url &other) const {
    return (m_tls == other.m_tls && m_scheme == other.m_scheme &&
            m_host == other.m_host && m_url == other.m_url &&
            m_port == other.m_port);
}
...
bool xmrig::Url::parse(const char *url) {
    ...
    if (p) {
        if (strncasecmp(url, kStratumTcp, sizeof(kStratumTcp) - 1) == 0) {
            m_scheme = STRATUM;
            m_tls = false;
        } else if (strncasecmp(url, kStratumSsl, sizeof(kStratumSsl) - 1) ==
                   0) {
            m_scheme = STRATUM;
            m_tls = true;
...
#ifdef XMRIG_FEATURE_HTTP
        else if (strncasecmp(url, kDaemonHttps, sizeof(kDaemonHttps) - 1) ==
                 0) {
            m_scheme = DAEMON;
            m_tls = true;
        } else if (strncasecmp(url, kDaemonHttp, sizeof(kDaemonHttp) - 1) ==
                   0) {
            m_scheme = DAEMON;
            m_tls = false;
        }
#endif
        else if (strncasecmp(url, websocket, sizeof(websocket) - 1) == 0) {
            m_scheme = WEBSOCKET;
        } else {
            return false;
        }
